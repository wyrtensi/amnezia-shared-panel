# Cloudflare setup: deploy the panel behind Access + Google

End-to-end runbook to expose the panel at **`<PANEL_DOMAIN>`** on the panel host —
behind a Cloudflare Tunnel and Cloudflare Access (Google login, email allowlist)
— **without opening any inbound port** and **without touching** any VPN stack
already on that box (a co-located node keeps its own `amnezia-awg2` /
`amnezia-awg3` / `amnezia-node-agent` containers and UDP ports untouched).

This ties together the pieces the other docs own — see
[`docs/ROLLOUT.md`](./ROLLOUT.md) (the end-to-end rollout),
[`docs/HOSTING.md`](./HOSTING.md) (architecture),
[`docs/CLOUDFLARE-ACCESS.md`](./CLOUDFLARE-ACCESS.md) (Access app detail +
two-way sync), and [`docs/UPDATE-MECHANISM.md`](./UPDATE-MECHANISM.md) (updates).
...
> from `.env.example`). The panel runs from the published image, on **loopback
> only** — defaults `127.0.0.1:5430` (web) / `127.0.0.1:5431` (control-api), both
> overridable via `WEB_PUBLISH_PORT` / `CONTROL_API_PUBLISH_PORT`. Postgres is
> never published.

**Shared machine — do NOT disturb what is already running.** The box may host an
unrelated Amnezia install or other tenants (their own `amnezia-awg2`,
`amnezia-awg3`, an Outline/`shadowbox` server, `watchtower`, a node-agent, etc.).
**Never stop, remove, or re-port them.** The panel comes up as its own Compose
project (`amnezia-panel`) on **uncommon loopback ports** so nothing collides. Pick
any free ports for the panel; the other containers keep theirs.

**Co-located node (panel + Amnezia node on one machine).** Supported and common.
Two ways for the panel's `control-api`/`worker` to reach a node-agent on the same
host — see [`ROLLOUT.md`](./ROLLOUT.md#5-co-location-networking):

- **Attach to the node's Docker network** (preferred) via a Compose override, then
  register the node with **`apiBaseUrl: http://amnezia-node-agent:4001`**.
- **Host route** — the panel services already get a `host.docker.internal` route
  (`infra/prod/compose.yaml`), so a node-agent published on the host loopback is
  reachable at **`apiBaseUrl: http://host.docker.internal:4001`**.

## TL;DR

- **Point the domain at `http://localhost:5430`** — the `web` container, published
  on host loopback as `127.0.0.1:5430` (override with `WEB_PUBLISH_PORT`). **Not**
  the control-api port, **not 443** (TLS terminates at Cloudflare's edge).
- **Transport is one Cloudflare Tunnel** (`cloudflared`, run as a host systemd
  service): `<PANEL_DOMAIN>` → `http://localhost:5430`. Zero inbound ports; the
  origin IP stays hidden; any co-located VPN stack is left exactly as-is.
...
| --- | --- | --- |
| `<PANEL_DOMAIN>` | Panel public hostname, e.g. `panel.example.com` | You pick it; its zone must be on Cloudflare |
| `<ZONE>` | The DNS zone, e.g. `example.com` | Your Cloudflare account |
...
| `<CF_API_TOKEN>` | Bearer management token for the write-back | My Profile → API Tokens |
...
1. **A Cloudflare account** with the **`<ZONE>` zone** already added, so Cloudflare
   can auto-write the tunnel's DNS record for `<PANEL_DOMAIN>`.
2. **Zero Trust enabled** on the account — pick a team name and select a plan
   ...
   > **Team domain is one per account.** `<TEAM>.cloudflareaccess.com` is shared by
   > **every** Access application on the account; the login page therefore shows the
   > account/team name, not the individual app. Each application is still fully
   > isolated by its own **AUD** and its own policy. See §4.4.
3. **A Google Cloud OAuth 2.0 client** (type *Web application*) in a project owned
   by your Google Workspace org — **Google Cloud Console → APIs & Services →
...
> firewall rule, and you will not touch a co-located node's containers or ports.

---

## 2. Deploy the panel on the server (compose)

The control plane is the `infra/prod` stack: `postgres`, a one-shot `migrate`,
`control-api` (`127.0.0.1:5431`), `worker`, and `web` (`127.0.0.1:5430`). Both
public-facing ports bind **loopback only** — the tunnel reaches them over
...
cd /opt/amnezia-panel/infra/prod
cp .env.example .env
```

Edit `infra/prod/.env` and set the real values:

```sh
# infra/prod/.env
PANEL_IMAGE=ghcr.io/<owner>/amnezia-shared-panel:latest
...
# First login by any of these becomes admin (comma-separated, lower-cased):
BOOTSTRAP_ADMIN_EMAILS=you@example.com,ops@example.com
# From section 4 (Access application) — required in production:
CF_ACCESS_ISSUER=https://<TEAM>.cloudflareaccess.com
CF_ACCESS_AUDIENCE=<AUD>
```

Postgres is never published to the host; inside the stack the app services reach
it as the compose service name `postgres`. `control-api` runs with
`NODE_ENV=production` (set in `compose.yaml`), which builds the Cloudflare Access
adapter and **requires** `CF_ACCESS_ISSUER` + `CF_ACCESS_AUDIENCE` (it throws at
boot if either is missing). You can fill the two `CF_ACCESS_*` values now if you
already have them, or come back after section 4.

### 2.2 Bring the stack up

```sh
cd /opt/amnezia-panel
bash infra/prod/update.sh          # backup → pull → migrate → up (data-safe)
```
...
```sh
curl -fsS http://127.0.0.1:5431/healthz && echo OK               # control-api
curl -fsS -o /dev/null -w '%{http_code}\n' http://127.0.0.1:5430/   # web
docker compose -f infra/prod/compose.yaml ps
```

This project is named `amnezia-panel` and is fully separate from any VPN compose
stack. No inbound port was opened.

> **One-click updates.** Run `sudo bash infra/prod/install-updater.sh` once so the
> **Update** button in Administration → Overview works — see
...
**What port do I point the domain to? → `http://localhost:5430`.** That is the
`web` container on `127.0.0.1:5430`. You do **not** create an `A` record to the
host's IP, and you open **no** inbound port. You install `cloudflared` on the box,
create one tunnel, and route `<PANEL_DOMAIN>` → `http://localhost:5430`.
Cloudflare writes the DNS for you.
...
(the box firewall is untouched, any co-located VPN stack keeps its exact
exposure), the **origin IP stays hidden**, and it runs as its **own systemd
service** — outside any watchtower's scope and outside the VPN compose stacks.

### 3.1 Create the tunnel + public hostname (dashboard, recommended)

In `one.dash.cloudflare.com` → **Networks → Tunnels**:

1. **Create a tunnel** → connector **Cloudflared** → name e.g. `panel`.
2. Copy the shown one-line **install command** (it embeds the tunnel token) and
   run it on the panel host. It installs and enables `cloudflared.service`; the
   connector then shows **HEALTHY**.
3. Open the tunnel's **Public Hostname** tab → **Add a public hostname**:
   - **Subdomain** the host part of `<PANEL_DOMAIN>`  **Domain** `<ZONE>`
     **Path** *(empty)*
   - **Service:** Type **HTTP**, URL **`localhost:5430`**
...
ingress:
  - hostname: <PANEL_DOMAIN>
    service: http://localhost:5430
  - service: http_status:404
```

```sh
cloudflared tunnel login
cloudflared tunnel create panel
cloudflared tunnel route dns panel <PANEL_DOMAIN>   # creates the CNAME
...
```
<PANEL_DOMAIN>   CNAME   <TUNNEL-UUID>.cfargotunnel.com   (Proxied / orange cloud)
```

The orange cloud is required: the hostname resolves to Cloudflare, so the panel
is reachable **with the VPN off** and Access can sit in front. (`<ZONE>` must be a
zone in the same account, per section 1.)

### 3.3 The one gotcha

Run `cloudflared` **on the host** (systemd), so `http://localhost:5430` resolves
to the loopback-published `web` port. If you instead run it in a container,
`localhost` is that container — you would need
`http://host.docker.internal:5430` (`--add-host host.docker.internal:host-gateway`)
...
All of this is edge/dashboard-side; none of it touches the panel host or any VPN
...
→ Google Workspace.** Paste the Google **Client ID** + **Client secret** from
section 1 → **Save** → **Test**.

> **Reusing one Google Workspace IdP across apps is safe** — the app only
> *references* the IdP. **Never edit the shared IdP** to suit one app: editing it
> affects every Access application that uses it. If this account already has a
> working Google Workspace login method, select it; do not create a duplicate or
> modify it.
...
curl -s -X POST "$BASE/accounts/<ACCOUNT_ID>/access/identity_providers" \
  ...
  --data '{
    ...
    "type": "google-apps",
    "config": { "client_id": "<GOOGLE_CLIENT_ID>", "client_secret": "<GOOGLE_CLIENT_SECRET>", "apps_domain": "<ZONE>" }
  }'
```
Response `result.id` = `<GOOGLE_IDP_ID>` (pins the app to Google in step 4.2). Use
`"type": "google-apps"` (with `config.apps_domain`) to lock login to one Workspace
domain at the IdP layer; `"type": "google"` accepts individual/Gmail accounts.
...
1. **Application name:** `Shared Panel`.
2. **Destinations → Public hostname:** the host part of `<PANEL_DOMAIN>`, domain
   `<ZONE>`, **Path empty**. Add no second hostname.
3. **Session Duration:** `1 week`.
4. **Identity providers:** select **Google Workspace** only; **Accept all identity
...
curl -s -X POST "$BASE/accounts/<ACCOUNT_ID>/access/apps" \
  ...
  --data '{
    ...
    "type": "self_hosted",
    "domain": "<PANEL_DOMAIN>",
    "destinations": [{ "type": "public", "uri": "<PANEL_DOMAIN>" }],
    ...
    "policies": [
      {
        "name": "Shared Panel management",
        "decision": "allow",
        "precedence": 1,
        "include": [
          { "email_domain": { "domain": "<ZONE>" } },
...
- **Whole domain:** `{ "email_domain": { "domain": "<ZONE>" } }`
- **One address:** `{ "email": { "email": "person@example.com" } }`
...
- **Issuer / team domain — Dashboard:** Zero Trust → Settings → Custom Pages →
  ...
  > This is the **account-wide** team domain — shared by every app on the account.
  > Two panels on the same account share the same `CF_ACCESS_ISSUER` and differ
  > only by `CF_ACCESS_AUDIENCE`. The AUD is what isolates them.
- **AUD tag — Dashboard:** Access → Applications → Shared Panel → Overview →
...
Put the two values from section 4 into `infra/prod/.env` (section 2.1), then
recreate `control-api`:
...
login that lands to `admin` — there is no seed script.

**Smoke test — with the VPN OFF**, open `https://<PANEL_DOMAIN>`:
...
> **Not** an Access **service token.** A service token (two custom request headers,
> `CF-Access-Client-Id` + `CF-Access-Client-Secret`) only gets a machine *past*
> the Access gate and grants **zero** management ability. Do not use one here.
...
| `updatePolicy()` | `PUT …/access/apps/<APP_ID>/policies/<POLICY_ID>` | `Access: Apps and Policies` — **Edit** |
...
calls; there is no tighter *named* account group that grants the write.

- **Where it is used:** only by the worker's **panel→Access write-back** (the
  two-way sync). The write-back reads the token + account/app/policy IDs from the
  **panel admin settings** (Administration → Policy → Cloudflare Access, or the CLI
  `cf-config` + `cf-token`), stored **encrypted at rest and write-only** — not from
  a committed `.env`.
...
- **Adjacent (optional), for a truly end-to-end raise that also creates/proxies
  the DNS record:** add **DNS — Edit** (Zone-scoped, the `<ZONE>` zone) +
...
1. `systemctl status cloudflared` on the panel host → active; the tunnel shows
   **HEALTHY** in the dashboard.
2. `docker compose -f infra/prod/compose.yaml ps` → all panel services up;
   `curl http://127.0.0.1:5430/` on the box returns 200.
3. From a network **with the VPN off**, `https://<PANEL_DOMAIN>` lands on the
   Cloudflare Access / Google screen, then the panel loads for an allowlisted
   account; a non-listed account is blocked at the edge.
4. Any co-located node's `awg` handshakes and node-agent are unaffected — no
   firewall rule was changed and no inbound port was opened.

## Related documents

- [`docs/ROLLOUT.md`](./ROLLOUT.md) — the end-to-end rollout (Cloudflare + nodes + panel).
- [`docs/HOSTING.md`](./HOSTING.md) — architecture and identity model.
- [`docs/CLOUDFLARE-ACCESS.md`](./CLOUDFLARE-ACCESS.md) — Access app detail,
  allowlist, and two-way panel↔Access sync (write-back + deactivation reconcile).
- [`docs/UPDATE-MECHANISM.md`](./UPDATE-MECHANISM.md) — the in-panel Update button.
</content>
</invoke>
