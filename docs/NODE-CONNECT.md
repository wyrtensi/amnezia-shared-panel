...
| `add-node.sh` fails re-registering a node | `nodes.name` has a unique index (`nodes_name_unique` in `packages/db/src/schema.ts`), so a second node under the same name can never be created — there is no duplicate row to clean up | Refused as `409 NODE_EXISTS` (an opaque `500` before this branch fixed unique-violation detection on the driver's wrapped error). Pick a different name, or `node-update` the existing node instead of re-adding it. |
