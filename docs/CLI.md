...
| `maintenanceIntervalSec` | 3600 s | 3600..604800 | Traffic roll-ups plus pruning — and, in the same run, two more consequential steps: it hard-deletes disabled accounts past `offboardedUserRetentionDays` (`purgeOffboardedUsers`) and re-arms keys stuck in `revoking` whose revoke exhausted its retries (`rearmStuckRevokes`). Lengthening this period also lengthens how promptly those two act, not just the roll-up cadence. Every run loads **every** `peer_samples` row in the raw retention window into the worker's heap at once and walks it twice; the container runs under a 160 MB `mem_limit`. The floor is the hour this loop always ran on, and it stays there until the roll-up aggregates in SQL |
...
| `offboardedUserRetentionDays` | 30 days | 1..3650 | How long a disabled account (and its revoked keys) is kept before `purgeOffboardedUsers` hard-deletes it, measured from `disabled_at`. Exists so the Restore button and `reinstate` stay usable for a while after an offboard — before this setting the account was gone within the hour (`maintenanceIntervalSec`'s own floor). A row with no `disabled_at` (disabled before this column existed) is **never** purged, regardless of this window, and a key still `revoking` still blocks the purge the same as it always did — see [Cloudflare Access](./CLOUDFLARE-ACCESS.md) for the reinstate flow itself |
