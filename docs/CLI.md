...
| `user-notices-reset <id\|email> [--notice=install\|update]` | Put one user back in front of the panel's two interruptions: the **install step** after a new key, and the **update poster** in front of an existing one (the signature sheet on the QR, the clipboard and both config files). Both unless `--notice=` names one. These are counted on the user row (`install_notice_acks`, `update_notice_acks`), which is the point of the command — the counters are monotonic, nothing else lowers them, and before they existed the only way to make the install step reappear was to delete the person's key rows. That is also the bug they fix: the install step used to be inferred from the key's per-owner number, which is `max + 1` and therefore **falls** when an admin purges that owner's revoked rows, so a routine cleanup handed somebody on their fortieth key a first-key warning. Reachable from the Users page as well. **It cannot clear the browser's own quiet period**: after somebody signs, the update notice stays down for ten minutes in that browser, and a reset inside that window looks to them like nothing happened — the counter really is back to zero, the poster returns once those ten minutes are up |
...
| `keys-purge-revoked [<id\|email>] [--confirm]` | **The bulk half of `key-purge`**, and the shell half of the panel's "Purge revoked" buttons — with no user named it sweeps the whole panel, with one it sweeps that person. Revoking deliberately keeps the row (reconcile finds an orphaned peer by the label that row holds), so on a panel that has been running a while the rows accumulate; this is what clears them. It purges only keys already in `revoked`, and each one goes through the same per-key route as `key-purge`, in order, so every purge is validated and audited separately and a key whose state moved under you is refused rather than forced; the command exits non-zero if any did. Irreversible: afterwards only the `admin.keys.purge` events say those keys existed. Without `--confirm` it prints the whole list — id, owner, device, note, node — and changes nothing |
...
| `node-reconcile <id>` | Queue a read-only inventory check: the worker compares the node's live peers against the keys the panel manages (matched by public key first, the deterministic label only as a fallback) and never deletes anything itself. The command only confirms the job was queued — the summary (managed/observed/matched/missing/orphan counts) lands in the audit log (`audit`) as a `node.reconcile` event once the worker runs it, usually within a few seconds. Keys stuck in `revoking` (a delete that never completed) are counted separately there as `revokingKeyCount` and `strandedRevokingPeerCount`; a non-zero `strandedRevokingPeerCount` means the peer is still on the node — retry the delete on that key rather than waiting on reconcile, which only ever reports it |
...
| `cf-sync [--status] [--json]` | Ask the outbox to reconcile the Cloudflare Access allowlist now, instead of waiting for the hourly timer or the next panel-side user change. Refuses up front, without queueing anything, when Cloudflare is not configured (`cf-config` / `cf-token`). `--status` shows the last run as one line, including a run that refused to act (Cloudflare unconfigured, or the blast-radius cap tripped) — that finishes as `failed` with the reason, same as any other failure; `--status --json` returns the raw status object. This is also where a **policy that is not attached to the Access application** surfaces: the panel reads a reusable policy fine but refuses to manage one that gates nothing, and says so here by name (see [`docs/CLOUDFLARE-ACCESS.md` §A.3.1](../docs/CLOUDFLARE-ACCESS.md)). Both policy kinds are written correctly — app-scoped through the application endpoint, reusable through the account one — so no setting distinguishes them |
...
| `maintenanceIntervalSec` | 3600 s | 3600..604800 | Traffic roll-ups plus pruning — and, in the same run, two more consequential steps: when `autoPurgeOffboardedUsers` is on (**off by default** — see `policy-set` above), it hard-deletes disabled accounts past `offboardedUserRetentionDays` (`purgeOffboardedUsers`); it also always re-arms keys stuck in `revoking` whose revoke exhausted its retries (`rearmStuckRevokes`), which is unrelated to that toggle. Lengthening this period also lengthens how promptly those two act, not just the roll-up cadence. Every run loads **every** `peer_samples` row in the raw retention window into the worker's heap at once and walks it twice; the container runs under a 160 MB `mem_limit`. The floor is the hour this loop always ran on, and it stays there until the roll-up aggregates in SQL |
...
| `offboardedUserRetentionDays` | 30 days | 1..3650 | How long a disabled account (and its revoked keys) is kept before it becomes eligible for deletion, measured from `disabled_at`. Only ever hard-deletes it automatically (`purgeOffboardedUsers`) when `autoPurgeOffboardedUsers` is on (**off by default**). Exists so the Restore button and `reinstate` stay usable for a while after an offboard. A row with no `disabled_at` (disabled before this column existed) is **never** purged, regardless of this window, and a key still `revoking` still blocks the purge the same as it always did — see [Cloudflare Access](./CLOUDFLARE-ACCESS.md) for the reinstate flow itself |
| `completedJobRetentionDays` | 30 days | 1..3650 | How long a `completed` `job_outbox` row is kept before `deleteCompletedJobsBefore` removes it, measured from `completed_at`. A `failed`, `pending` or `processing` row is never touched by this window — a failed row is the evidence an operator reads to see what went wrong, and the stuck-revoke re-arm sweep below counts them. The `rules.refresh` and `access.sync` singleton rows are always kept regardless of status or age; deleting either would make the admin UI report those jobs as idle and lose the last run's error |
...
uses its own. A value set in the panel wins over both. The other five periods
...
1. the active feed payload's addresses;
2. **minus** the admin exclusions (exact match);
...
amnezia-panel global-routes-set --profile=ru_blacklist \
  --exclude-cidrs=203.0.113.0/24 --add-cidrs=198.51.100.0/24
```

Entries are validated exactly like per-user custom routes (bare IPs normalized
to host routes), with a 2000-entry cap per list. The audit log records only the
resulting counts, never the entries.
...
They are refused rather than accepted-and-ignored because a script that
"succeeds" and changes nothing is worse than one that stops. The control API
refuses the same payload, so this is only the earlier, clearer half of it.

A deployment that stored site names before this rule keeps them: `global-routes`
