 ...
 * step.
 *
 * Three, because the reminder is aimed at the person who has not done it yet,
 * and by the third key somebody has either installed the client or is not going
 * to be convinced by a fourth dialog. It is a count of keys **ever created**,
 * not of keys currently held — see `keyNumberOf`.
 */
export const INSTALL_REMINDER_KEYS = 3;
 ...
 * which is the honest reading of "their first three keys": counting the keys
 * they hold today would show the dialog forever to anyone who revokes as they
 * go. The one thing that can lower it is an administrator **purging** a key row
 ...
 * definition belong to users who are long past their third key) — those get no
 * reminder rather than a guessed number.
 */
