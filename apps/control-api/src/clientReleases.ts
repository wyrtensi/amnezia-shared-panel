...
// Three iOS listings. Which of the first two a user can install depends on
// where their Apple account is registered: AmneziaVPN is the real client; it
// is hidden from the Russian App Store by Roskomnadzor requirement (and from
// the Chinese one), which is why DefaultVPN -- the same developers under
// another listing name -- exists and is the primary button here. AmneziaWG is
// a third, separate app -- a plain WireGuard-style client, not a fork of
// either of the above -- offered everywhere, so it carries no such caveat. The
// guide says all three, in that order.
...
const toClientRelease = (release: GitHubRelease, now: Date): ClientRelease => {
  ...
  const downloads: ClientPlatformDownload[] = [
    ...
    },
    {
      ...
      secondAlternate: storeAsset(AMNEZIAWG_APP_STORE_URL),
    },
  ];
