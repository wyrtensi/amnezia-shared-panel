...
#   2. install Docker Engine + Compose v2 on the node, if missing
#   3. copy infra/node to the node, create its layout, secret, and .env, and
#      authorize the panel's SSH key — the image ship in step 4 already needs it
#   4. ship the pinned node-agent image and record its ID *as loaded there*
#   5. repair state permissions, then run the node's own deploy.sh (which runs
#      preflight.sh itself — invoking it twice fails the RAM gate on a small host)
#   6. allocate a tunnel port and install the supervised autossh unit
#   7. register the node through the bundled admin CLI and reconcile it
#
...
# --- 2. Docker on the node --------------------------------------------------
say "[2/7] Ensuring Docker Engine + Compose v2 on the node"
...
# --- 3. Node layout, secret, and .env ---------------------------------------
say "[3/7] Installing infra/node into ${NODE_DIR} on the node"
...
# --- 3b. Authorize the panel's key on the node ------------------------------
...
# --- 4. Ship the node-agent image -------------------------------------------
say "[4/7] Shipping the node-agent image"
...
say "[5/7] Running the node's preflight and deploy"
...
# --- 6. Supervised tunnel on the panel host ---------------------------------
say "[6/7] Opening the panel -> node tunnel"
...
# --- 7. Register in the panel -----------------------------------------------
say "[7/7] Registering the node in the panel"
